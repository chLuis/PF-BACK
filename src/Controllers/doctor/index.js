...
export const postDoctor = async (req, res) => {
    try {
        ...
        const user = new Usuario({_id, idLink: _idDoctor , nombre, apellido, dni, direccion, telefono, mail, fechaNacimiento, password: passwordHashed, medico})
        await user.save()
        const doctor = new Doctor({_id: _idDoctor, usuario_id: _id, matricula, especialidad, aprobado})
        await doctor.save()
...
export const deleteDoctor = async (req, res) => {
    try{
        ...
        const {nombre, apellido} = await Usuario.findByIdAndDelete(usuario_id)
        res.status(200).send(`Eliminaste a ${nombre} ${apellido} ${especialidad} de la base de datos`)
    }
    catch (error) {
        res.status(400).send(error.message)
    }
