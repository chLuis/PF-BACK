...
export const postPaciente = async (req, res) => {
    try {
        ...
        const user = new Usuario({_id, idLink: _idPaciente, nombre, apellido, dni, direccion, telefono, mail, fechaNacimiento, password: passwordHashed})
        await user.save()
        const paciente = new Paciente({_id: _idPaciente, usuario_id: _id, obraSocial})
        await paciente.save()
